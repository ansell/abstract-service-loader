...
/**
 * An abstract service loader based on the model that there is a single key for each service, but
 * each key may be provided in more than one service.
 * 
 * Services are automatically loaded in the constructors, and additional services can be added
 * programmatically.
 * 
...
public abstract class AbstractServiceLoader<K, S>
{
    ...
    /**
     * Loads instances of the currently available services into a cache in memory using the given
     * ClassLoader.
     * 
     * @param serviceClass
     *            The service class to search for.
     ...
     */
    protected AbstractServiceLoader(final Class<S> serviceClass, final ClassLoader classLoader)
    {
        // Java6 version:
        final ServiceLoader<S> serviceLoader = java.util.ServiceLoader.load(serviceClass, classLoader);
        
        final Iterator<S> servicesIterator = serviceLoader.iterator();
        
        // Java5 version:
        // Iterator<S> services =
        // javax.imageio.spi.ServiceRegistry.lookupProviders(serviceClass, classLoader);
        ...
        while(true)
        {
            try
            {
                if(!servicesIterator.hasNext())
                {
                    ...
                    {
                        this.log.info("Completed loading {} services for class {}", this.services.size(),
            ...
            // This error is thrown when .hasNext() or .next() encounter an invalid service
    ...
    public void add(final S service)
    {
        ...
        if(set == null)
        {
            ...
            {
                set = existingSet;
                this.log.info("Found duplicate service with key: {} service class: {}", key, service.getClass()
                        .getName());
            }
        }
    ...
    public Collection<S> getAll()
    {
        ...
        for(final Collection<S> nextServices : new ArrayList<Collection<S>>(this.services.values()))
        {
     ...
     * @return True if there is at least one registered service with the given key, and false
     *         otherwise.
     */
